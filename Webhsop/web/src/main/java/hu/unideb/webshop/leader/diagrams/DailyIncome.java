...
public class DailyIncome implements Serializable {
    ...
    public void update() {
        ...
        for (IncomeDTO currentIncome : incomes) {
            if (currentIncome.getOrderId() == null) {
                continue;
            }
            String name = "";
            int quantity = 0;
            if (currentIncome.getComment() != null) {
                String[] s = currentIncome.getComment().split(",");
                if (s.length >= 2) {
                    name = s[0];
            ...
            Data tmp = new Data(name, quantity, currentIncome.getPrice());
            allMoney += tmp.getCost();
            if (currentIncome.getOrderId() == 0l) {
